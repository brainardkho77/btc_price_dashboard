...
## Derivatives Impact
- Best with derivatives: `hgb` at `55.0%`. Best without derivatives: `hgb` at `56.0%`. Improved directional accuracy: `False`.

## Feature Group Watchlist
- `dollar_rates_only` / `logistic_linear`: samples `100`, accuracy `52.0%`, Sharpe `0.83`, drawdown `-42.9%`, Brier `0.263`, calibration error `0.110`, reliability `Low confidence`.
...
- `dollar_rates_only` / `logistic_linear`: `2` stable slices
