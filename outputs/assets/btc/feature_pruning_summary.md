...
## Run Summary
- Asset: `BTC`
- Run ID: `btc_research_20260507T123724Z`
- Selected model: `no_valid_edge`
- Signal: `neutral`
- Reliability: `Low confidence`
- Current all-feature best ML: `logistic_linear` at `49.0%` accuracy, Brier `0.267`, net return `353.4%`.
- Best baseline: `momentum_90d` at `55.0%` accuracy.

## Best Pruned Candidate
- `all_features_official` / `logistic_linear`: samples `100`, accuracy `49.0%`, Brier `0.267`, calibration `0.111`, Sharpe `0.59`, drawdown `-66.6%`, net return `353.4%`.
- Stability evidence: bootstrap lower bound `40.0%`, bootstrap upper bound `59.0%`, permutation p-value `0.665`.
...
- `derivatives_sum_open_interest_z_365d` (`derivatives_only`): IC `-0.268`, p-value `0.029`, coverage `64.0%`, keep `True`.
- `onchain_supply_z_365d` (`onchain_only`): IC `-0.257`, p-value `0.009`, coverage `100.0%`, keep `True`.
- `derivatives_taker_buy_sell_ratio_z_365d` (`derivatives_only`): IC `-0.256`, p-value `0.037`, coverage `64.0%`, keep `True`.
- `derivatives_taker_buy_sell_ratio_z_90d` (`derivatives_only`): IC `-0.253`, p-value `0.038`, coverage `65.0%`, keep `True`.
