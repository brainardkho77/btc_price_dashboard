...
## Summary
- Full refresh run ID: `btc_research_20260511T151538Z`
...
Quick mode used `2018-01-01` to `2026-05-11` with `75` 30d official samples and selected `random_forest`. Full refresh used `2015-01-01` to `2026-05-11` with `100` 30d official samples and selected `no_valid_edge`. The quick edge disappeared because the broader 2015+ sample added regimes where the ML models did not beat the 90d momentum baseline or the full baseline set after costs.

## 30d Official Model Comparison
- Best ML model: `hgb` with directional accuracy `51.0%`
- Best baseline: `momentum_90d` with directional accuracy `55.0%`
- Baselines that beat the best ML model: `momentum_90d, momentum_30d`
...
- `etf_flows` / `momentum_90d`: samples `100`, accuracy `55.0%`, net return `840.4%`, reliability `Low confidence`
...
- `random_forest` in `post_halving`: samples `24`, accuracy `62.5%`, net return `584.8%`, reliability `Low confidence`
...
- `hgb` in `low_volatility`: samples `66`, accuracy `57.6%`, net return `858.5%`, reliability `Low confidence`
- `hgb` in `high_rate`: samples `66`, accuracy `56.1%`, net return `152.5%`, reliability `Low confidence`
- `momentum_30d` in `low_rate`: samples `34`, accuracy `55.9%`, net return `361.2%`, reliability `Low confidence`
- `random_forest` in `low_rate`: samples `34`, accuracy `55.9%`, net return `431.4%`, reliability `Low confidence`
- `hgb` in `post_etf`: samples `27`, accuracy `55.6%`, net return `112.3%`, reliability `Low confidence`
