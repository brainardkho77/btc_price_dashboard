...
## SOL Selection Audit
- `all_features` / `logistic_linear`: accuracy `69.4%`, after-cost return `1240.2%`, conclusion `current_official_reference`.
- `price_plus_risk_assets` / `logistic_linear`: accuracy `69.4%`, after-cost return `759.2%`, conclusion `accuracy_not_enough_without_reference_improvement`.
