...
## Run Summary
- Asset: `SOL`
- Run ID: `sol_research_20260511T154021Z`
- Selected model: `logistic_linear`
- Signal: `neutral`
- Reliability: `Medium confidence`
- Current all-feature best ML: `logistic_linear` at `69.4%` accuracy, Brier `0.246`, net return `982.7%`.
- Best baseline: `random_permutation` at `55.6%` accuracy.

## Best Pruned Candidate
- `all_features_official` / `logistic_linear`: samples `36`, accuracy `69.4%`, Brier `0.246`, calibration `0.103`, Sharpe `1.32`, drawdown `-37.4%`, net return `982.7%`.
...
- `all_features_official` / `hgb`: label `promising_but_rejected`, accuracy `63.9%`, net return `540.3%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
- `all_features_official` / `random_forest`: label `promising_but_rejected`, accuracy `52.8%`, net return `392.0%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
