...
## Summary
- Full refresh run ID: `spx_research_20260529T102518Z`
...
- Best ML model: `hgb` with directional accuracy `65.7%`
...
- `hgb`: did_not_beat_buy_hold; low_reliability
- `random_forest`: did_not_beat_buy_hold; failed_transaction_cost_check; low_reliability

## Feature Group Findings
- `risk_assets_only` / `random_forest`: samples `99`, accuracy `68.7%`, net return `212.8%`, reliability `Medium confidence`
...
- `hgb` in `low_rate`: samples `34`, accuracy `70.6%`, net return `88.7%`, reliability `Low confidence`
