...
## Run Summary
- Asset: `BTC`
- Run ID: `btc_research_20260512T082912Z`
- Selected model: `no_valid_edge`
- Signal: `neutral`
- Reliability: `Low confidence`
- Current all-feature best ML: `hgb` at `49.0%` accuracy, Brier `0.259`, net return `536.2%`.
- Best baseline: `momentum_90d` at `55.0%` accuracy.

## Best Pruned Candidate
- `all_features_official` / `hgb`: samples `100`, accuracy `49.0%`, Brier `0.259`, calibration `0.086`, Sharpe `0.65`, drawdown `-59.7%`, net return `536.2%`.
- Stability evidence: bootstrap lower bound `39.0%`, bootstrap upper bound `59.0%`, permutation p-value `0.683`.
...
- `all_features_official` / `hgb`: label `promising_but_rejected`, accuracy `49.0%`, net return `536.2%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
- `all_features_official` / `logistic_linear`: label `promising_but_rejected`, accuracy `49.0%`, net return `277.8%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
- `all_features_official` / `random_forest`: label `promising_but_rejected`, accuracy `48.0%`, net return `452.4%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
...
- `derivatives_sum_open_interest_z_365d` (`derivatives_only`): IC `-0.285`, p-value `0.019`, coverage `64.0%`, keep `True`.
