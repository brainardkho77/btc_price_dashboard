...
## Run Summary
- Asset: `BTC`
- Run ID: `btc_research_20260512T102621Z`
- Selected model: `no_valid_edge`
- Signal: `neutral`
- Reliability: `Low confidence`
- Current all-feature best ML: `hgb` at `49.0%` accuracy, Brier `0.260`, net return `536.2%`.
- Best baseline: `momentum_90d` at `55.0%` accuracy.

## Best Pruned Candidate
- `all_features_official` / `hgb`: samples `100`, accuracy `49.0%`, Brier `0.260`, calibration `0.086`, Sharpe `0.65`, drawdown `-59.7%`, net return `536.2%`.
...
- `all_features_official` / `logistic_linear`: label `promising_but_rejected`, accuracy `49.0%`, net return `270.9%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
- `all_features_official` / `random_forest`: label `promising_but_rejected`, accuracy `47.0%`, net return `383.4%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
...
- `derivatives_sum_open_interest_z_365d` (`derivatives_only`): IC `-0.270`, p-value `0.027`, coverage `64.0%`, keep `True`.
