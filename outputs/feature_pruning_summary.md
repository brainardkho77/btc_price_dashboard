...
- Current all-feature best ML: `random_forest` at `54.0%` accuracy, Brier `0.245`, net return `1086.8%`.
...
- `derivatives_sum_open_interest_z_90d` (`derivatives_only`): IC `-0.216`, p-value `0.076`, coverage `66.0%`, keep `True`.
- `cross_asset_spx_ret_30d` (`risk_assets_only`): IC `-0.210`, p-value `0.034`, coverage `100.0%`, keep `True`.
- `derivatives_taker_buy_sell_ratio_z_90d` (`derivatives_only`): IC `-0.206`, p-value `0.095`, coverage `65.0%`, keep `True`.
- `macro_reverse_repo_chg_7d` (`macro_liquidity_only`): IC `0.198`, p-value `0.046`, coverage `100.0%`, keep `True`.
- `derivatives_sum_open_interest` (`derivatives_only`): IC `-0.192`, p-value `0.114`, coverage `67.0%`, keep `True`.
