...
- Current all-feature best ML: `hgb` at `51.0%` accuracy, Brier `0.256`, net return `446.8%`.
- Best baseline: `momentum_90d` at `55.0%` accuracy.

## Best Pruned Candidate
- `all_features_official` / `hgb`: samples `100`, accuracy `51.0%`, Brier `0.256`, calibration `0.087`, Sharpe `0.63`, drawdown `-45.6%`, net return `446.8%`.
- Stability evidence: bootstrap lower bound `42.0%`, bootstrap upper bound `60.5%`, permutation p-value `0.515`.
...
- `all_features_official` / `hgb`: label `promising_but_rejected`, accuracy `51.0%`, net return `446.8%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
- `all_features_official` / `random_forest`: label `promising_but_rejected`, accuracy `49.0%`, net return `185.9%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
- `all_features_official` / `logistic_linear`: label `promising_but_rejected`, accuracy `46.0%`, net return `14.6%`, CI low `n/a`, p-value `n/a`, reason `current_all_features_reference`.
...
- `derivatives_sum_open_interest_value` (`derivatives_only`): IC `-0.358`, p-value `0.002`, coverage `67.0%`, keep `True`.
...
- `derivatives_sum_open_interest_z_365d` (`derivatives_only`): IC `-0.286`, p-value `0.019`, coverage `64.0%`, keep `True`.
- `derivatives_taker_buy_sell_ratio_z_90d` (`derivatives_only`): IC `-0.263`, p-value `0.031`, coverage `65.0%`, keep `True`.
...
- `derivatives_taker_buy_sell_ratio_z_365d` (`derivatives_only`): IC `-0.242`, p-value `0.050`, coverage `64.0%`, keep `True`.
