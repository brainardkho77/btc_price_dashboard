...
## Summary
- Full refresh run ID: `btc_research_20260506T125607Z`
...
- Best ML model: `hgb` with directional accuracy `55.0%`
...
- `hgb`: did_not_beat_90d_momentum; low_reliability
...
- `derivatives_only` / `logistic_linear`: samples `45`, accuracy `55.6%`, net return `234.8%`, reliability `Low confidence`
...
- `dollar_rates_only` / `logistic_linear`: samples `100`, accuracy `52.0%`, Sharpe `0.83`, max drawdown `-42.9%`, Brier `0.263`, calibration error `0.110`, beats buy-hold `True`, beats momentum `False`.
...
- `hgb` in `bear_market`: samples `45`, accuracy `62.2%`, net return `602.2%`, reliability `Low confidence`
- `momentum_90d` in `high_rate`: samples `66`, accuracy `62.1%`, net return `267.2%`, reliability `Low confidence`
- `hgb` in `low_volatility`: samples `66`, accuracy `60.6%`, net return `1582.8%`, reliability `Low confidence`
- `momentum_90d` in `bear_market`: samples `45`, accuracy `57.8%`, net return `64.0%`, reliability `Low confidence`
- `hgb` in `high_rate`: samples `66`, accuracy `57.6%`, net return `236.3%`, reliability `Low confidence`
- `hgb` in `pre_etf`: samples `73`, accuracy `56.2%`, net return `595.0%`, reliability `Low confidence`
...
- Best derivatives impact row: `random_forest`, derivatives included `True`, accuracy `60.6%`.
