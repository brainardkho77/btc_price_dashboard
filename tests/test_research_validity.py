...
def test_btc_candidate_rescue_audit_keeps_regime_slices_diagnostic_only():
    ...
    audit = btc_candidate_rescue_audit(
        "btc_research_test",
        raw,
        feature_result,
        config,
    ...
    validate_frame("btc_candidate_rescue_audit.csv", audit)
    assert not audit.empty
    assert {"overall_candidate", "signal_policy_rescue"}.issubset(set(audit["audit_scope"]))
    regime_rows = audit[audit["audit_scope"] == "regime_slice"]
    assert not regime_rows.empty
    assert regime_rows["final_test_ranked"].eq(True).all()
